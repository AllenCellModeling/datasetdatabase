...
class DatasetDatabase(object):
    ...
    def __init__(self,
        ...
        self.schema_version = version.SchemaVersion(self)
        self.driver = config[list(config.keys())[0]]["driver"]
        self.tables = self.schema_version.get_tables()
    ...
    def construct_tables(self):
        ...
        # construct tables
        self.schema_version.create_schema()

        # add basic info
        self.schema_version.add_basic_info()
    ...
    def _create_iota(self,
        ...
        group_info = self._insert_to_table("Group", group)
        iota_group_items = []
        ...
        for key, value in row.iteritems():
            ...
            iota_group = {"IotaId": iota_info["IotaId"],
                          "GroupId": group_info["GroupId"],
                          "Created": datetime.now()}
            iota_group_info = self._insert_to_table("IotaGroupJunction",
                                                    iota_group)

            bar.increment()

        group_dataset = {"GroupId": group_info["GroupId"],
                         "DatasetId": ds_info["DatasetId"],
                         "Created": datetime.now()}
        group_dataset_info = self._insert_to_table("GroupDatasetJunction",
                                                   group_dataset)
    ...
    def _create_dataset(self, params) -> dict:
        ...
        if filepath_columns is not None and store_files:
            if not force_storage:
                # calc storage
                total_size = 0
                dataset[filepath_columns].apply(
                    lambda fp: total_size += os.path.getsize(fp))
        ...
        # create dataset
        ds_info = self._insert_to_table("Dataset",
            {"Name": name,
             "Description": description,
             "SourceId": source_info["SourceId"],
        ...
        # begin iota creation
        print("\nCreating Iota...")
        iota_bar = tools.ProgressBar(len(dataset) * len(dataset.columns))
        dataset.apply(
            lambda row: self._create_iota(row, ds_info, iota_bar), axis=1)
